...
**Microsoft Store 提出版は「モデル非同梱」＝約3GB**。アプリは初回の生成時に
HuggingFace からモデルをダウンロードする。spec の既定もこれに合わせて**非同梱**。
→ 通常はこの章の作業は不要。`pyinstaller MultiVoiceStudio.spec` をそのまま実行すればよい。

> ⚠️ 過去に「`models/` があれば自動同梱」する spec だったため、開発で prefetch した
> だけで誤って約17GBのMSIXが出来る事故があった。現在は **`MVS_BUNDLE_MODELS=1` を
> 明示したときだけ同梱**する（既定は非同梱）。

### （任意）モデル同梱のオフライン動作版を作りたいとき（約15-18GB）

ネット不要で動く版が必要な場合のみ、以下を行ってから
`MVS_BUNDLE_MODELS=1` を立ててビルドする（`$env:MVS_BUNDLE_MODELS="1"`）。
...
- **既定（モデル非同梱・Store提出版）= 約 3GB。** 初回の生成時にモデルをDLする。
- オプトイン（`MVS_BUNDLE_MODELS=1`・オフライン同梱版）= **約 15〜18GB**。Store 上限内だが大きく、初回 DL/インストールが重い。
